...
namespace Better_Pumpkin_Fusion_Bep
{
    ...
    public class Core : BasePlugin
    {
        ...
        {
            [HarmonyPrefix]
            public static bool Summon(SuperMachineNut __instance)
            {
                if (!Input.GetKey(KeyCode.LeftShift))
                    return true;
                foreach (Plant plant in Board.Instance.plantArray.ToArray().Where(plant =>
                                                                                  plant != null &&
                                                                                  plant.thePlantColumn == __instance.thePlantColumn &&
                                                                                  plant.thePlantRow == __instance.thePlantRow &&
                                                                                  plant.thePlantType == PlantType.Pumpkin))
                {
                    __instance.thePlantHealth -= 8000;
                    SetPlant_Patch.SetPlant(__instance.thePlantColumn, __instance.thePlantRow, PlantType.WallNut);
                    return false;
                }
                return true;
            }
